...
from config_regression import config, OUTPUT_DIR, DATA_FLAGS, TARGET_COL
...
from models.regression_models import (
    get_regression_models, TabPFNICLWrapper, 
    GPT2ICLRegressorWrapper, create_hybrid_ensemble
)
from regression_utils import log_timing_info, display_results_table, save_results
...
from feature_engineering.plotting_utils import (
    ...
    save_metrics_and_predictions, plot_fold_performance_comparison
)
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('train_regression.log')
    ]
)
logger = logging.getLogger(__name__)

def run_regression_pipeline(X, y, config, output_dir):
    logger.info("="*50)
    logger.info("REGRESSION PIPELINE STARTING")
    logger.info("="*50)
    ...
    base_models = get_regression_models(config.RANDOM_STATE, model_names=config.MODELS_TO_TEST)
    ...
    outer_cv = KFold(n_splits=config.CV_FOLDS, shuffle=True, random_state=config.RANDOM_STATE)
    ...
    for fold, (train_idx, test_idx) in enumerate(outer_cv.split(X), 1):
        ...
        for model_name, base_model in base_models.items():
            ...
            try:
                ...
                plot_pred_vs_true(y_test_outer, y_pred, model_analysis_dir, model_name, task_type="regression")
                plot_residuals(y_test_outer, y_pred, model_analysis_dir, model_name, task_type="regression")
                plot_prediction_error_analysis(y_test_outer, y_pred, model_name, model_analysis_dir, task_type="regression")
                
                if hasattr(model, 'coef_') or hasattr(model, 'feature_importances_'):
                    feature_names = [f"PCA_Component_{i}" for i in range(X_train_sel.shape[1])]
                    plot_feature_importance_analysis(model, feature_names, model_analysis_dir, model_name, task_type="regression")
                
                try:
                    plot_learning_curves_detailed(X_train_sel, y_train_outer, model, model_analysis_dir, model_name, task_type="regression")
                ...
                save_metrics_and_predictions(
                    ...
                    f"{model_name}_fold_{fold}", task_type="regression"
                )
                ...
                results[model_name]['r2_scores'].append(0.0)
                results[model_name]['mae_scores'].append(0.0)
                results[model_name]['mse_scores'].append(0.0)
        ...
        for model_name in base_models.keys():
            if len(results[model_name]['r2_scores']) > 0:
                latest_r2 = results[model_name]['r2_scores'][-1]
                latest_mae = results[model_name]['mae_scores'][-1]
                latest_mse = results[model_name]['mse_scores'][-1]
                ...
                logger.info(f"{model_name}: {status} R² = {latest_r2:.3f}, MAE = {latest_mae:.3f}, MSE = {latest_mse:.3f}, Time = {latest_time:.2f}s")
        ...
        for model_name in base_models.keys():
            if len(results[model_name]['r2_scores']) > 0:
                score = results[model_name]['r2_scores'][-1]
                if score > best_score:
                    best_score = score
    ...
    if config.ENSEMBLE_METHODS and len(results) > 0:
        ...
        if ensemble_models:
            ...
            for ensemble_name, ensemble_model in ensemble_models.items():
                ...
                for fold, (train_idx, test_idx) in enumerate(outer_cv.split(X), 1):
                    ...
                    ensemble_results['r2_scores'].append(r2)
                    ensemble_results['mae_scores'].append(mae)
                    ensemble_results['mse_scores'].append(mse)
                    ensemble_results['training_times'].append(training_time)
                    
                    logger.info(f"R² = {r2:.3f}, MAE = {mae:.3f}, MSE = {mse:.3f}, Time = {training_time:.2f}s")
                
                results[ensemble_name] = ensemble_results
                
                mean_r2 = np.mean(ensemble_results['r2_scores'])
                std_r2 = np.std(ensemble_results['r2_scores'])
                mean_mae = np.mean(ensemble_results['mae_scores'])
                std_mae = np.std(ensemble_results['mae_scores'])
                mean_mse = np.mean(ensemble_results['mse_scores'])
                std_mse = np.std(ensemble_results['mse_scores'])
                mean_time = np.mean(ensemble_results['training_times'])
                
                logger.info(f"{ensemble_name} Performance:")
                logger.info(f"R² = {mean_r2:.3f} ± {std_r2:.3f}")
                logger.info(f"MAE = {mean_mae:.3f} ± {std_mae:.3f}")
                logger.info(f"MSE = {mean_mse:.3f} ± {std_mse:.3f}")
        ...
        try:
            ...
            if final_results_data:
                ...
                final_results_df = final_results_df.sort_values('Mean_R2', ascending=False)
                ...
                if len(individual_models) > 0 and len(ensemble_models) > 0:
                    logger.info("Final vs stacking comparison plot")
        except Exception as e:
            logger.warning(f"Could not create final vs stacking comparison plot: {e}")
    ...
    if not results_df.empty:
        ...
        logger.info(f"Best R² score: {results_df.iloc[0]['Mean_R2']:.3f} ± {results_df.iloc[0]['Std_R2']:.3f}")
        ...
        if best_model_name in config.MODELS_WITH_HYPERPARAMETERS:
            ...
            if best_model_name == "TabPFNICL":
                final_model = TabPFNICLWrapper(random_state=config.RANDOM_STATE)
            elif best_model_name == "GPT2ICL":
                final_model = GPT2ICLRegressorWrapper()
            else:
                final_model = base_models[best_model_name]
            ...
            if best_model_name == "HybridEnsemble":
                ...
                for model_name, result in results.items():
                    if len(result['r2_scores']) > 0 and not model_name.endswith('Ensemble'):
                        mean_r2 = np.mean(result['r2_scores'])
                        temp_results_data.append({'Model': model_name, 'Mean_R2': mean_r2})
                ...
                for model_name, result in results.items():
                    if len(result['r2_scores']) > 0 and not model_name.endswith('Ensemble'):
                        score = np.mean(result['r2_scores'])
                        if score > best_score:
                            best_score = score
        ...
        try:
            ...
            if final_results_data:
                final_results_df = pd.DataFrame(final_results_data)
                final_results_df = final_results_df.sort_values('Mean_R2', ascending=False)
                plot_final_summary(final_results_df, os.path.join(output_dir, "analysis"), metric="r2", task_type="regression")
                logger.info("Final summary plot")
        except Exception as e:
...
def create_comprehensive_analysis_plots(results, output_dir):
    ...
    if results_data:
        ...
        plot_learning_curves_comparison(results_df, analysis_dir, task_type="regression")
        plot_statistical_significance_matrix(results_df, analysis_dir, task_type="regression")
        plot_model_complexity_analysis(results_df, analysis_dir, task_type="regression")
        ...
        if len(ensemble_models) > 0:
            ...
            plot_ensemble_analysis(results_df, ensemble_dict, analysis_dir, task_type="regression")
            plot_ensemble_details(ensemble_dict, analysis_dir, task_type="regression")
        
        logger.info("Comprehensive analysis plots")
...
def main():
    ...
    try:
        ...
        output_dir = f"{OUTPUT_DIR}/regression_comparison_{timestamp}"
        ...
        logger.info(f"Model comparison plots: {os.path.join(output_dir, 'analysis', 'model_comparison_r2.png')}")
