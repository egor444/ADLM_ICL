...
def test_model(model, data_path, target_col='age', use_pca=None):
    ...
    if use_pca:
        test_data = use_pca.transform(test_data.drop(columns=['eid', target_col]))
    else:
        test_data = test_data.drop(columns=['eid', target_col]).values
    X_test = test_data
...
def run_regression():
    ...
    data_path = "../data/regression/train_fat.csv"
    test_path_wat = "../data/regression/test_fat.csv"
    test_path_fat = "../data/regression/test_fat.csv"
    USE_PCA = True
...
def run_test_classification():
    ...
    y_pred = model.predict(X_test)
    print("\tModel testing complete. Calculating metrics..")
    ...
    plt.savefig('outputs/tabpfn_classification_confusion_matrix.png')
    plt.show()
    print("TabPFN classification test finished.")
    return accuracy
