...
config = {
    ...
    "FEATURE_SELECTION_STRATEGIES": {
        "multi_method_scoring": False,         # Use all methods (univariate, MI, LASSO)
        "hierarchical_filtering": True,       # Use variance/correlation filtering
        "group_based_selection": False         # Balance features across groups
    },
}
...
def test_icl_with_localized_selection(dm):
    ...
    for i in range(len(X_test)):
        ...
        # Nearest neighbors
        idx_orig = retrieve_neighbors(x_query, X_train, k_original)
        idx_inv = retrieve_neighbors(x_query, x_inv, k_inverse)
        ...
        model = TabPFNRegressor(device='cuda' if torch.cuda.is_available() else 'cpu', random_state=config["RANDOM_STATE"])
        ...
        preds_tabpfn.append(y_pred[0])
        maes_tabpfn.append(mae)

        if i % 100 == 0:
            logging.info(f"Test sample {i+1}/{len(X_test)}:\n\tTabpfn: Predicted={y_pred[0]}, Actual={y_true}, MAE={mae}")

    preds_tabpfn = np.array(preds_tabpfn)
    maes_tabpfn = np.array(maes_tabpfn)
    logging.info(f"Overall MAEs: TabPFN: {mean_absolute_error(y_test, preds_tabpfn)}, R2: {r2_score(y_test, preds_tabpfn)}")

    # plotting results
    title = f'TabPFN Regression Predictions vs Actual, r2: {r2_score(y_test, preds_tabpfn)}, MAE: {mean_absolute_error(y_test, preds_tabpfn)}'
    plot_predictions(y_test, preds_tabpfn, title, 'results/icl_results/tabpfn_regression_predictions.png')
    logging.info("TabPFN regression with localized selection completed.")
    # save test predictions
    results_df = pd.DataFrame({
        'Actual': y_test,
        'Predicted': preds_tabpfn,
        'MAE': maes_tabpfn
    })
    results_df.to_csv('results/icl_results/test_set/tabpfn_regression_results.csv', index=False)
...
def process_single_sample(j, X_test, X_train, y_train, x_inv, y_inv, y_test, nn_orig, nn_inv):
    ...
    model = TabPFNRegressor(device='cuda' if torch.cuda.is_available() else 'cpu', random_state=config["RANDOM_STATE"] + j)
    model.fit(x_context, y_context)
...
def test_icl_folds(dm):
    ...
    for i in range(fold_count):
        ...
        whole_fold_tabpfn = TabPFNRegressor(device='cuda' if torch.cuda.is_available() else 'cpu', random_state=config["RANDOM_STATE"] + i)
    ...
    fold_preds_df.to_csv('results/icl_results/tabpfn_fold_predictions.csv', index=False)
