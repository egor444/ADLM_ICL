...
if __name__ == "__main__":
    ...
    if args.test_data_mgr:
        ...
        dm = DataManager("classification", "liver", "emb", "rboth", "verbose", logger=d_logger)
        split_data = dm.get_fold_data_set([0, 1])
        d_logger.info(f"Split data shape: {split_data.shape} of folds 0 and 1")
        fold_loader = dm.get_fold_data_loader([2, 3], batch_size=32, shuffle=True)
        d_logger.info(f"Fold data loader created with {len(fold_loader)} batches")
        first_batch = next(iter(fold_loader))
        d_logger.info(f"First batch shape: {first_batch[0].shape}, target: {first_batch[1].shape}")
        four_splits = dm.get_fold_data_set([0, 1, 2, 4])
        d_logger.info(f"Four splits shape: {four_splits.shape}")
        single_split = dm.get_fold_data_set([4])
        d_logger.info(f"Single split shape: {single_split.shape}")
        single_split = dm.get_fold_data_set([3])
        d_logger.info(f"Single split shape: {single_split.shape}")
        d_logger.info(f"Id_splits: {dm.fold_data_indices}")
